...
//{{APP_CLASS_DECL
public class HTML_ReposMaint extends HTML_ReposMaintBase
//END_APP_CLASS_DECL}}
{
    // HTML_ReposMaint Class Constructor
	//{{APP_CLASS_CTOR
	public HTML_ReposMaint ()
	//END_APP_CLASS_CTOR}}
    {
	    super();
    }
    ...
    public synchronized PLSResponse doAction(PLSRequest preq)
    {
		//{{APP_DOACTION
		   ...
		   if ( VSTrace.IS_ON )
		     begTime = (tr = VSTrace.get()).set(VST_SESSION_ID, getSessionIDForTracing()).set(VST_CATEGORY, VST_PLS_ACTION).set(VST_ACTION_NAME,"HTML_ReposMaint.doAction").beg();
